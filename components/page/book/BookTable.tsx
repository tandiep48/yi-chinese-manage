...
// components/page/book/BookTable.tsx
// Book data table with edit/delete actions.
      ...
      <table className="min-w-full divide-y divide-slate-100">
        <thead className="bg-slate-50">
          <tr>
            ...
            <th className={th}>English Name</th>
            <th className={th}>Vietnamese Name</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {loading ? (
            ...
            items.map((b) => (
              ...
              >
                <td className={td}>
                  <span className="font-mono text-xs font-medium text-slate-800">{b.book_code}</span>
                </td>
                <td className={td}>{b.name_en || "—"}</td>
                <td className={td}>{b.name_vn || "—"}</td>
                <td className={`${td} text-right`}>
                  {deleteCode === b.book_code ? (
                    <span className="inline-flex items-center gap-2">
                      <span className="text-xs text-slate-500">Delete?</span>
                      <button
                        id={`book-confirm-delete-${b.book_code}`}
                        onClick={() => { onDelete(b); setDeleteCode(null); }}
                    ...
                    <span className="inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        id={`book-delete-${b.book_code}`}
                        onClick={() => setDeleteCode(b.book_code)}
                        title="Delete"
