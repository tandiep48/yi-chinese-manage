...
// components/page/grammar_context/GrammarContextTable.tsx
// Grammar context data table with edit/delete actions.
...
import type { GrammarContext } from "@/lib/types/grammar";

interface GrammarContextTableProps {
  ...
  onEdit: (context: GrammarContext) => void;
  onDelete: (context: GrammarContext) => void;
}
...
function previewJson(value: GrammarContext["content_json"], max = 70): string {
  if (value == null) return "—";
  const text = JSON.stringify(value);
  return text.length > max ? `${text.slice(0, max)}…` : text;
}

export function GrammarContextTable({
  items,
  total,
  totalPages,
  page,
        ...
        <table className="min-w-full divide-y divide-slate-100">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className={th}>Content JSON</th>
            ...
            {loading ? (
              ...
              items.map((c) => (
                ...
                >
                  ...
                  <td className={td}>
                    <span className="font-medium text-slate-800">{c.grammar_id}</span>
                  </td>
                  <td className={`${td} font-mono text-xs text-slate-500`}>
                    {previewJson(c.content_json)}
                  </td>
                  <td className={`${td} text-right`}>
                    {deleteId === c.id ? (
                      <span className="inline-flex items-center gap-2">
                        ...
                        <button
                          id={`grammar-context-confirm-delete-${c.id}`}
                          onClick={() => { onDelete(c); setDeleteId(null); }}
                      ...
                      <span className="inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          id={`grammar-context-delete-${c.id}`}
