  ...
  function launchTraining(engine: TrainerEngine, types: string[]) {
    ...
    try {
      if (engine === "vocab") {
        ...
        router.push(`/vocab-training-batch?mode=6&passage_id=${encodeURIComponent(passageId)}`);
      } else {
        sessionStorage.setItem("lessonTrainerActivityTypes", JSON.stringify(types));
        router.push(`/lesson-training?passage_id=${encodeURIComponent(passageId)}`);
      }
    } catch {
      ...
      router.push(
        engine === "vocab"
          ...
          : `/lesson-training?passage_id=${encodeURIComponent(passageId)}`
      );
    }
  }
    ...
    <LessonStudyShell passageId={passageId} domain="lesson">
      ...
      {!passageId ? (
        ...
        <WordSummary
          ...
          loading={loading}
          error={vocabError}
          onLearn={() => router.push(`/vocab-learning?passage_id=${encodeURIComponent(passageId)}&flow=lesson-part`)}
        ...
        <LessonSummary
          ...
          loading={loading}
          error={error}
          onLearn={() => router.push(`/lesson-learning?passage_id=${encodeURIComponent(passageId)}`)}
